...
//$statment2=$connection->query("UPDATE `slot` SET `states` ='yes' WHERE `slot`.`id` = '$id'");
 $statment3=$connection->query("INSERT INTO `all_order` (`time`,`datesend`,`timesend`,`phoneuser`, `phoneworker`, `tokenuser`, `tokenworker`, `description`, `id`, `date`, `timestart`, `timeend`, `Am_Pm`, `username`, `state`,`orderimage`,`accept`,`start`,`deleteornot`) VALUES ('$time','$datesend','$timesend','$phoneuser', '$phoneworker', '$tokenuser', '$tokenworker', '$description', '$id', '$date', '$timestart' , '$timeend','$Am_Pm', '$username','notfinished','$imagename','no','no','no')"); 
